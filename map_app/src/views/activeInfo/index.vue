<template>
  <div class="map-wrap">
    <div class="map-header">
      <div class="left-icon" @click="$router.replace({ name: $route.query.back? $route.query.back:'active' })">
    ...
    <div class="map-content">
      <div class="user">
        <h4>{{ data.promoter && data.promoter.nickname }}<span v-if="data.promoter._id === user._id" class="tag">ME</span><span class="tag">{{status[1]}}</span></h4>
        <span>{{ data.createdAt | dateFormat }}</span><br>
        <span>start date: {{ data.startDate | dateFormat }}</span><br>
        <span>end date: {{ data.endDate | dateFormat }}</span>
      </div>
      <div class="content" v-html="data.content"></div>
      <div class="post-content">
        <span @click.stop="onAddActive" class="action" v-if="user._id !==data.promoter._id && !isJoin &&status[0]!==2">
          <van-icon name="plus" />
        </span>
