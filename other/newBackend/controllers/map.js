...
const startRoute = async (req, res) => {
	try {
		...
		const _route = await Route.findOne({
		...
		if (!_route) {
			...
			const _update = await Route.findOneAndUpdate(
				{
					user: req.session.user,
					origin: req.body.origin,
					destination: req.body.destination,
	...
	try {
		...
		if (!_route) {
			...
			if (_route['completed'][_route['totalTrial'] - 1] == 'NOTYET') {
				...
				const _new_route = await Route.findOneAndUpdate(
					{
						user: req.session.user,
						origin: req.body.origin,
						destination: req.body.destination,
				...
				const _new_user = await User.findOneAndUpdate(
