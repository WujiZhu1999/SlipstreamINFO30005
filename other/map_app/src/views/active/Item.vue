<template>
	<div class="post">
		<p class="header">
			{{ item.promoter && item.promoter.nickname
			}}<span v-if="item.promoter._id === user._id" class="tag">ME</span
		...
		<div class="post-content">
			<span
				...
				class="action"
				v-if="user._id !== item.promoter._id && !isJoin && status[0] !== 2"
			>
				<van-icon name="plus" />
			</span>
			<span
			...
			<span
				@click.stop="
					$router.push({ name: 'editActive', query: { _id: item._id } })
				"
				v-if="isMyposts"
				class="action"
			>
