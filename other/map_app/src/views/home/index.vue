    ...
    <div class="map-header">
      ...
      <div class="action">
        ...
        <div v-show="routes[routeIndex]">
          <a v-if="!isRiding" @click="onStartStroke" class="start-btn" :style="{color:'rgb(25, 137, 250)'}" href="javascript:;;">Start stroke</a>
          <a v-else-if="currentRoute.status === 0" @click="onEndStroke" class="start-btn" :style="{color:'rgb(25, 137, 250)'}" href="javascript:;;">End stroke</a>
          <a v-else-if="currentRoute.status === 1" :style="{color:'#333'}" class="start-btn" href="javascript:;;">Completed</a>
        </div>
      </div>
    </div>
    <div class="map-info" v-if="routes[routeIndex]">
      <div>
        <van-icon name="peer-pay" /> Whole length <span style="color:#1989fa">{{routes[routeIndex].legs[0].distance.text}}</span>
      </div>
      <div>
        <van-icon name="tosend" /> Required time <span style="color:#1989fa">{{routes[routeIndex].legs[0].duration.text}}</span>
      </div>
      <div>
        <van-icon name="circle" /> Intersection number <span style="color:#1989fa">{{routes[routeIndex].legs[0].steps.length }}</span>
      </div>
      <div>
        <van-icon name="other-pay" /> Proportion of bicycle lanes <span style="color:#1989fa">{{getBicycle(routes[routeIndex].legs[0].steps) }}</span>
      </div>
      <div>
        <van-icon name="award-o" /> {{(routes[routeIndex].legs[0].distance.value*0.3/1000).toFixed(2)}}%of trees you saved
      </div>
      <div>
      ...
      <div class="lins" v-if="!currentRoute._id">
        <template v-for="item in routes.length">
          <a :style="{color:routeIndex===item-1?'rgb(25, 137, 250)':'#999'}" v-if="item <=3" href="javascript:;;" :key="item" @click="onSelectRouteIndex(item-1)">Route {{item}}</a>
        </template>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    onLocation () {
       ...
       * 
       */
      window.location.href = "https://www.google.com/maps/dir/?api=1&origin=" + this.urlreplace(this.start) + "&destination=" + this.urlreplace(this.end) + "&travelmode=bicycling";
    },
    urlreplace (str) {
    ...
    getBicycle (arr) {
      ...
      const bicyLen = arr.filter(item => item.travel_mode === "BICYCLING").length
