...
const localRoute = async (req, res) => {
	if (!req.session.user) {
        ...
        */
		return res.render('map/maphome2', {
	...
	try {
		...
		if (!route) {
			...
			if (route['totalTrial'] != 0) {
				...
				const ret = await Route.findOneAndDelete({
					user: req.session.user,
					origin: req.body.origin,
					destination: req.body.destination,
				});
...
const startRoute = async (req, res) => {
	try {
		...
		const found_route = await Route.findOne({
			user: req.session.user,
			origin: req.body.a_origin,
			destination: req.body.a_end,
		});
		if (!found_route) {
			...
			if (found_route['status'][found_route['totalTrial'] - 1] == 'WAIT') {
				...
				const route_update = await Route.findOneAndUpdate(
					{
						user: req.session.user,
						origin: req.body.a_origin,
						destination: req.body.a_end,
				...
				const route_updatee = await Route.findOne({
					user: req.session.user,
					origin: req.body.a_origin,
					destination: req.body.a_end,
				});
				...
				found_route['completed'][found_route['totalTrial'] - 1] == 'NOTYET'
			) {
				var d = new Date();
...
const haltRoute = async (req, res) => {
	try {
		...
		if (!found_route) {
			return res.send(
				'No such route found, make sure your destination/origin are spelled correctly.',
			);
		} else {
			...
			if (found_route['completed'][found_route['totalTrial'] - 1] == 'NOTYET') {
				...
				return res.send(
					'Already up to date, lastest route halted/stopped Already',
				);
			}
			...
			route_updatee = await Route.findOne({
				user: req.session.user,
				origin: req.body.a_origin,
				destination: req.body.a_end,
			});
	...
	try {
		...
		if (!found_route) {
			...
			if (found_route['completed'][found_route['totalTrial'] - 1] == 'NOTYET') {
				...
				const update_new_route = await Route.findOneAndUpdate(
					{
						user: req.session.user,
						origin: req.body.origin,
						destination: req.body.destination,
				...
				const update_new_user = await User.findOneAndUpdate(
					{ userName: req.session.user },
					found_user,
				);
				//render
				return res.render('map/end', {
					route: found_route,
					userName: req.session.user,
				});
			} else {
