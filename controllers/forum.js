...
const getForum = async (req, res) => {
	try {
		...
		for (article of found_articles) {
			...
			article['Hrs'] = Math.floor(
				Math.abs(new Date() - new Date(article['time'])) / (1000 * 3600),
			);
			article['Mins'] =
				Math.floor(
					Math.abs(new Date() - new Date(article['time'])) / (1000 * 60),
				) -
				Math.floor(
					Math.abs(new Date() - new Date(article['time'])) / (1000 * 3600),
				) *
					60;
			article['Secs'] =
				Math.floor(Math.abs(new Date() - new Date(article['time'])) / 1000) -
				Math.floor(
					Math.abs(new Date() - new Date(article['time'])) / (1000 * 60),
				) *
					60;
	...
	try {
		...
		if (found_article) {
			for (comment of found_article.comments) {
				...
				comment['Hrs'] = Math.floor(
					Math.abs(new Date() - new Date(comment['time'])) / (1000 * 3600),
				);
				comment['Mins'] =
					Math.floor(
						Math.abs(new Date() - new Date(comment['time'])) / (1000 * 60),
					) -
					Math.floor(
						Math.abs(new Date() - new Date(comment['time'])) / (1000 * 3600),
					) *
						60;
				comment['Secs'] =
					Math.floor(Math.abs(new Date() - new Date(comment['time'])) / 1000) -
					Math.floor(
						Math.abs(new Date() - new Date(comment['time'])) / (1000 * 60),
					) *
						60;
...
const createArticle = async (req, res) => {
	...
	if (
		...
		return res.render('error', {
			error:
				'There is incomplete data for article. Make sure you include a title and body',
			redirect: '/forum',
		});
	}
...
const getEditArticle = async (req, res) => {
	try {
		...
		const found_article = await Article.findOne({
			articleNum: req.params.articleNum,
		});
		//render info
...
const createComment = async (req, res) => {
	try {
		...
		if (!found_article) {
			...
			const comment_update = await Article.findOneAndUpdate(
				{ articleNum: enteredNumber },
				{ comments: comments },
			);
...
const changeComment = async (req, res) => {
	...
	try {
		...
		if (!flag) {
			...
			const article_update = await Article.findOneAndUpdate(
