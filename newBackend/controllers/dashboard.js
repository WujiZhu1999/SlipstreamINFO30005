...
const getHomepage = async (req, res) => {
	try {
		...
		if (output !== 'USERNOTFOUND') {
			...
			_lead.push(_me);
			_lead.sort((a, b) =>
				a.data.totalDistance >= b.data.totalDistance ? -1 : 1,
			);
			output += _lead;
...
async function getStats(req, res) {
	try {
		...
		const output =
			...
			user.name +
			'</dd><dd> DISTANCE: ' +
			user.data.totalDistance +
			'</dd><dd>' +
			user.data.energySaved +
			' KJ energy Saved </dd>' +
			'<dd>' +
			user.data.carbonSaved +
			' G carbon Saved </dd>' +
			'<dd>' +
			user.data.streak +
			' days Streak </dd>';
		...
		res.status(400);
		return res.send(
			'Failure when fetching personal information for main page(dashboard)',
		);
	}
}
